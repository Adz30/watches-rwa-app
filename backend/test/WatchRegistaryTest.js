...
describe("WatchRegistry", function () {
  ...
  describe("Minting Watches", function () {
    ...
    it("should NOT allow non-owner to mint", async function () {
      ...
      await expect(
        watchRegistry.connect(alice).mintWatch(alice.address, uri)
      ).to.be.reverted;
    });

    it("should increment tokenIdCounter for each mint", async function () {
      await watchRegistry.connect(owner).mintWatch(alice.address, "ipfs://rolex.json");
      await watchRegistry.connect(owner).mintWatch(bob.address, "ipfs://omega.json");
